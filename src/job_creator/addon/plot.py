...
def analyze_argv(arg):
    ...
    if fname == "-h":
        ...
        print("-catc: show file content clean (comments removed)")
        print("-k 'labelnr or name' (-o dest.txt): kill a column and save")
    ...
    if option0 == "-p":
        ...
        if "-e" not in llist:
            print("you need an option for the error: -e label(s)")
        else:
            xarg = llist.index("-e")
            try:
                e = llist[xarg + 1]
                if y in labels:
                    ...
                    try:
                        eind = int(e)
                        if eind >= len(labels) or eind < 0:
                            print(e, "is too big or small")
                            return -1
                    except:
                        print(e, "is no label of file 1")
                        return -1
                
            except:
                print("you need to specify a e-label")
                print("possible labels are:")
                for i in range(n_labels):
                    print(i,":",labels[i])
                return -1
        ...
        if "-param" in llist:
            ...
            eimed = data[eind]
            summed = [0]
            for i in range(len(yimed)):
                summed[i] += float(yimed[i])
                summed.append(summed[i])
                data[yind][i] = float(yimed[i])
                data[xind][i] = float(ximed[i])
                data[eind][i] = float(eimed[i])
                #~ print(i+1, summed[i])
            summed.pop()
            spaceing = 1
            #~ pylab.plot(data[xind][begin:end][0::spaceing], summed[begin:end][0::spaceing], "r^-")
            #~ pylab.plot(data[xind][begin:end][0::spaceing], data[yind][begin:end][0::spaceing], "r^-")
            pylab.errorbar(data[xind][begin:end][0::spaceing]
                         , data[yind][begin:end][0::spaceing]
                         , data[eind][begin:end][0::spaceing]
                         , 0
                         , "r-"
                         , ecolor = "b"
                         )
        ...
        if "-o" in llist:
            arg = llist.index("-o")
            pylab.savefig(llist[arg + 1])
        else:
            pylab.savefig("res.svg")
