                        ...
                        if(stage2_[0][i][j] == 1) {
                            std::for_each(grow_dir_.begin(), grow_dir_.end(), 
                                [&](uint const & dir) {
                                    switch(dir) {
                                        case(qmc::down):
                                            if(stage2_[0][(i + 1) % H_][j] == 0)
                                                stage2_[0][(i + 1) % H_][j] = 2;
                                            break;
                                        case(qmc::right):
                                            if(stage2_[0][i][(j + 1) % L_] == 0 and qmc::n_bonds != qmc::hex)
                                                stage2_[0][i][(j + 1) % L_] = 2;
                                            break;
                                        case(qmc::diag_down):
                                            if(stage2_[0][(i + 1) % H_][(j + 1) % L_] == 0 and qmc::n_bonds == qmc::tri)
                                            stage2_[0][(i + 1) % H_][(j + 1) % L_] = 2;
                                            break;
                                        case(qmc::diag_up):
                                            if(stage2_[0][(i + H_ - 1) % H_][(j + L_ - 1) % L_] == 0 and qmc::n_bonds == qmc::tri)
                                                stage2_[0][(i + H_ - 1) % H_][(j + L_ - 1) % L_] = 2;
                                            break;
                                        case(qmc::left):
                                            if(stage2_[0][i][(j + L_ - 1) % L_] == 0 and qmc::n_bonds != qmc::hex)
                                                stage2_[0][i][(j + L_ - 1) % L_] = 2;
                                            break;
                                        case(qmc::up):
                                            if(stage2_[0][(i + H_ - 1) % H_][j] == 0)
                                                stage2_[0][(i + H_ - 1) % H_][j] = 2;
                                            break;
                                        case(qmc::hori):
                                            if((i+j) % 2 == 1) {
                                                if(stage2_[0][i][(j + L_ - 1) % L_] == 0  and qmc::n_bonds == qmc::hex)
    ...
    private:
        ...
        bool sanity_check() const {
            ...
            for(uint n = 0; n < N_; ++n) {
                for(uint i = 0; i < H_; ++i) {
                    for(uint j = 0; j < L_; ++j) {
                        if(stage2_[stage2_[n][i][j]][i][j] != n) {
                            std::cout << REDB << "sanity check failed in swap_region_class at state: " << n << " pos: " << i <<"/" << j << NONE << std::endl;
                            return false;
                        }
                    }
                }
